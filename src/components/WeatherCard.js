...
export default function WeatherCard({ forecast }) {
  if (!forecast?.forecast?.city) {
    return <p className="text-gray-300 text-center mt-4">No weather data available</p>;
  }
  ...
  return (
    <div className="w-full flex flex-col items-center mt-6">
      ...
      <div className="bg-gray-700 text-white p-6 rounded-xl shadow-lg w-full max-w-md mt-4 border border-emerald-700">
        <div className="text-center mt-3">
          <p className="text-4xl font-bold">{today.main.temp}°C</p>
          <p className="capitalize text-emerald-200">{today.weather[0].description}</p>
          <img
        ...
        <div className="grid grid-cols-2 gap-4 text-white text-sm mt-4">
      ...
      <h3 className="text-xl font-semibold text-emerald-300 mt-8">5-Day Forecast</h3>
      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-4 mt-4">
        {dailyForecasts.map((day, index) => (
          <Card key={index} className="p-4 w-50 bg-gray-900 border border-emerald-700 text-white shadow-lg text-center" >
            <CardHeader>
